...
namespace CarRental.Web.Core
{
	public class ApiControllerBase : ApiController, IServiceAwareController
	{
		...
		void IServiceAwareController.RegisterDisposableServices(List<IDisposable> disposableServices)
		{
			RegisterServices(disposableServices);
		}

		List<IDisposable> IServiceAwareController.DisposableServices
		{
			get
			{
